...
public class ModifyCustomerController
{
    ...
    public Customer showAddCustomer() throws IOException {
        ...
        fxmlLoader.setLocation(getClass().getResource("/views/modify-customer.fxml"));
    ...
    public void initialize() {
        List<String> countries = DataManager.countries.stream().map(c -> c.getCountryName()).collect(
            Collectors.toList());
        countryList.setItems(FXCollections.observableList(countries));
        countryList.getSelectionModel().selectFirst();

        List<String> divisionsByCountry = DataManager.getDivisionsByCountryName(countryList.getValue()).stream()
        ...
        countryList.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent ae) {
                ...
                List<String> divisionsByCountry = DataManager.getDivisionsByCountryName(countryName).stream()
                    .map(d -> d.getDivisionName()).collect(Collectors.toList());
                divisionList.setItems(FXCollections.observableList(divisionsByCountry));
                divisionList.getSelectionModel().selectFirst();
            }
    ...
    public void onConfirm() {
        ...
        Division division = DataManager.getDivisionByNameAndCountryName(divisionName, countryName);
        ...
        if (customerName.isEmpty() || customerAddress.isEmpty() ||
            ...
            if (isNew) {
                ...
                customer = new Customer(id, customerName, customerAddress, customerPostalCode, customerPhoneNumber, customer.getAppointments(), division);
